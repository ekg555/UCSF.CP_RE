...
library(tictoc)
tic('initialize Parse CofO Txt - parallel.R')
...
# =======================================================================================================
#   'getCO_Values(filepath)' -> 1-row dataframe.
# =======================================================================================================
#  for (i in 1:length(flist)) 
getCO_Values <- function(filepth) {

  Filenom <- filepth
  txt <- gsub("\\s+", " ", str_trim(readLines(filepth)) )
...
toc(log=TRUE) # PDF>>txt Initializing

tic("[txt>>table] OVERALL")
dat1 <- foreach(i=1:length(flist), .combine = 'rbind') %dopar% getCO_Values(flist[i])
